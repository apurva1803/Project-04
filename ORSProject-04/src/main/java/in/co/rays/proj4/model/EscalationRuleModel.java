...
public class EscalationRuleModel {
	...
	public Integer nextPk() throws DatabaseException {
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement("select max(id) from EscalationRule");
	...
	public long add(EscalationRuleBean bean) throws ApplicationException, DuplicateRecordException {
		...
		try {
			...
			PreparedStatement pstmt = conn
					.prepareStatement("insert into EscalationRule values(?, ?, ?, ?, ?, ?, ?, ?, ?)");
			pstmt.setInt(1, pk);
	...
	public void update(EscalationRuleBean bean) throws ApplicationException, DuplicateRecordException {
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement(
					"update EscalationRule set ruleCode = ?, level = ?, assignedTo = ?, status=?, created_by = ?, modified_by = ?, created_datetime = ?, modified_datetime = ? where id = ?");
	...
	public void delete(EscalationRuleBean bean) throws ApplicationException {
		...
		try {
			conn = JDBCDataSource.getConnection();
			conn.setAutoCommit(false);
			PreparedStatement pstmt = conn.prepareStatement("delete from EscalationRule where id = ?");
	...
	public EscalationRuleBean findByName(String name) throws ApplicationException {
		...
		StringBuffer sql = new StringBuffer("select * from EscalationRule where assignedTo = ?");
		EscalationRuleBean bean = null;
		Connection conn = null;
		try {
			conn = JDBCDataSource.getConnection();
	 ...
	 */
	public List<EscalationRuleBean> search(EscalationRuleBean bean, int pageNo, int pageSize)
