...
//ADD***************--------------------
	public long add(BroadcastBean bean) throws ApplicationException, DuplicateRecordException, SQLException{

		BroadcastBean duplicate = findByCode(bean.getCode());
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement("insert into st_broadcast values(?,?,?,?,?,?,?,?,?,?)");
			pstmt.setInt(1, pk);
			pstmt.setString(2, bean.getCode());
			pstmt.setString(3, bean.getMessage());
			pstmt.setString(4,bean.getContent());
			pstmt.setDate(5,new java.sql.Date(bean.getTime().getTime()));
	...
	//Delete*****************-----------------------------
	public void delete(BroadcastBean bean) throws SQLException, ApplicationException {
		Connection conn=null;
		try {
	...
	public void update(BroadcastBean bean) throws SQLException, ApplicationException, DuplicateRecordException {
		Connection conn=null;
		
		BroadcastBean duplicate = findByCode(bean.getCode()); // Check if updated Role
		...
		try {
			...
			conn=JDBCDataSource.getConnection();
			conn.setAutoCommit(false);
			PreparedStatement pstmt=conn.prepareStatement("update st_broadcast set code=? , message=?, content=?,time=? , status=? ,created_by=?,modified_by=?,created_datetime=?,modified_datetime=? where id=?");
	...
	//find by name***************************---------------------------------
	public BroadcastBean findByCode(String code) throws ApplicationException {
		StringBuffer sql=new StringBuffer("select * from st_broadcast where code=?");
		BroadcastBean bean=null;
		Connection conn=null;
		try {
			conn=JDBCDataSource.getConnection();
	...
	public List search(BroadcastBean bean) {
	...
	public List search(BroadcastBean bean, int pageNo, int pageSize) throws ApplicationException {
...
//list all data search
	...
	public List list(int pageNo, int pageSize) throws ApplicationException {
		...
		ArrayList list = new ArrayList();
		StringBuffer sql = new StringBuffer("select * from st_broadcast");
