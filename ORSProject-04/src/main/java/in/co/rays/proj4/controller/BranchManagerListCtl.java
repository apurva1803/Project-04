...
public class BranchManagerListCtl extends BaseCtl{
	 ...
	 @Override
	    ...
	    protected void doGet(HttpServletRequest request, HttpServletResponse response)
			...
			try {
				List<BranchManagerBean>list = model.search(bean, pageNo, pageSize);
				List<BranchManagerBean> next = model.search(bean, pageNo + 1, pageSize);
			...
			try {
				...
				if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
					...
					if (ids != null && ids.length > 0) {
						BranchManagerBean deletebean = new BranchManagerBean();
						for (String id : ids) {
	    ...
	    protected String getView() {
	        return ORSView.BRANCHMANAGER_LIST_VIEW;
	    }
}
