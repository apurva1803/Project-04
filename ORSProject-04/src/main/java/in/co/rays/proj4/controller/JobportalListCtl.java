...
@WebServlet(name = "JobportalListCtl", urlPatterns = { "/ctl/JobportalListCtl" })
public class JobportalListCtl extends BaseCtl{
		...
		protected void doGet(HttpServletRequest request, HttpServletResponse response)
			...
			try {
				List<JobportalBean> list = model.search(bean, pageNo, pageSize);
				List<JobportalBean> next = model.search(bean, pageNo + 1, pageSize);
			...
			try {
				...
				if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
					...
					if (ids != null && ids.length > 0) {
						...
						JobportalBean deletebean = new JobportalBean();
						for (String id : ids) {
					...
					ServletUtility.redirect(ORSView.JOBPORTAL_LIST_CTL, request, response);
					return;
		...
		protected String getView() {
			return ORSView.JOBPORTAL_LIST_VIEW;
		}
}
